...
3) Intersect the set of DEGs from MUT-D11 vs WT-D11 with the set of DEGs from WT-D11 vs WT-D7 to obtain the p1-specific set of reference genes: a reference gene should be significantly downregulated in MUT-D11 vs WT-D11 but significantly upregulated in WT-D11 vs WT-D7:

```{r, include=T}
p1.degs.mut.wt = as.data.frame(p1.mut.wt.res) %>%
  rownames_to_column(var = "gene_name") %>%
  filter(padj < fdr) %>%
  filter(log2FoldChange < 0) %>%
  pull(gene_name)

p1.degs.11.7 = as.data.frame(p1.11.7.res) %>%
  rownames_to_column(var = "gene_name") %>%
  filter(padj < fdr) %>%
  filter(log2FoldChange > 0) %>%
  pull(gene_name)

p1.ref.genes = intersect(p1.degs.mut.wt, p1.degs.11.7)
...
3) Intersect the set of DEGs from MUT-D11 vs WT-D11 with the set of DEGs from WT-D11 vs WT-D7 to obtain the p2-specific set of reference genes: a reference gene should be significantly downregulated in MUT-D11 vs WT-D11 but significantly upregulated in WT-D11 vs WT-D7:

```{r, include=T}
p2.degs.mut.wt = as.data.frame(p2.mut.wt.res) %>%
  rownames_to_column(var = "gene_name") %>%
  filter(padj < fdr) %>%
  filter(log2FoldChange < 0) %>%
  pull(gene_name)

p2.degs.11.7 = as.data.frame(p2.11.7.res) %>%
  rownames_to_column(var = "gene_name") %>%
  filter(padj < fdr) %>%
  filter(log2FoldChange > 0) %>%
  pull(gene_name)

p2.ref.genes = intersect(p2.degs.mut.wt, p2.degs.11.7)
...
```{r, include=T}
pM.degs.mut.wt = as.data.frame(pM.mut.wt.res) %>%
  rownames_to_column(var = "gene_name") %>%
  filter(padj < fdr) %>%
  filter(log2FoldChange < 0) %>%
  pull(gene_name)

pM.degs.11.7 = as.data.frame(pM.11.7.res) %>%
  rownames_to_column(var = "gene_name") %>%
  filter(padj < fdr) %>%
  filter(log2FoldChange > 0) %>%
  pull(gene_name)

pM.ref.genes = intersect(pM.degs.mut.wt, pM.degs.11.7)
