...
For either domain, the correlation coefficients of shared and specific region-gene pairs are comparable and hence the shared and specific pairs are likely to have comparable probabilities of being functional. The fact that the correlation coefficients of both the shared and the specific pairs are lower in p2 than in pMN could be explained by a difference in data quality (as we expect that the degree of correlation in real transcription regulation should not depend on a domain and be an intrinsic property of the fundamental regulation mechanisms).

Compare the correlation coefficients of the p2-specific pairs in p2 and pMN. For functional region-gene pairs that are specific to p2 we expect very low correlation in pMN (not a high correlation coefficient that just was not high enough to call a region-gene pair significant in pMN):
