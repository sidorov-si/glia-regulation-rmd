...
There is only one significant NFIA element -- NFIA-dependent gene pair in p1. It includes the gene Aldoc (Aldolase, Fructose-Bisphosphate C) which is an astroglial marker, according to the [CellMarker 2.0](http://117.50.127.228/CellMarker/CellMarkerDetail.jsp?species=Mouse&tissue_type=Brain&tissue_class=Brain&cell_name=Astrocyte&cell_type=Normal%20cell&marker=Aldoc&marker_source=Experiment&fourtable=four) database ([Hu et al., 2022](https://academic.oup.com/nar/article/51/D1/D870/6775381)) and which, in human, [is expressed almost exclusively in the brain](https://gtexportal.org/home/gene/ALDOC). Its significant connection to the Interval_33430 NFIA element (chr11:78152202-78154210) is also present in p2 and pM:
...
As there is only one significant NFIA element -- NFIA-dependent gene pair in p1, we will compare only p2 and pMN.
...
The two p2-specific gene-region pairs that are not present among the pM pairs both contain the _Actg2_ gene (Actin Gamma 2, Smooth Muscle). Looks irrelevant to gliogenesis, unless it plays a role in cell motility during the neural tube formation.

One of the two pM-specific gene-region pairs that are not present among the p2 pairs includes the _Cntnap3_ gene, Contactin Associated Protein Family Member 3, whose product belongs to the NCP protein family and hence may participate in neuron-glia interactions, according to the [NCBI Gene Summary](https://www.ncbi.nlm.nih.gov/gene/79937#summary). This gene-region pair looks relevant, but because the neuron-glia interactions could be important for the gliogenesis in any of the three domains (p1, p2 and pM), I would need to check if this pair was just a near miss of the set gene expression or region accessibility cutoff in p2 and p1 (in which case it may not be reasonable to classify it as p2-specific). The other pair includes the _Prr18_ gene, Proline Rich 18, whose relevance is unclear as it is not well characterised (according to [GeneCards](https://www.genecards.org/cgi-bin/carddisp.pl?gene=PRR18)).

Next, call p2- and pM-specific gene-region pairs which are also present in the opposite domain (passed the expression and accessibility cutoffs) but either the gene, or the region, or both are not NFIA-dependent, or, alternatively, both are dependent but the correlation between them is insignificant.

Find such p2-specific pairs:

```{r}
p2.conn.element.dep.pM.comm = p2.conn.element.dep %>%
  mutate(gene_region_id = paste(gene.ENSEMBL, RegionID, sep = "_")) %>%
  dplyr::rename("region_gene_correlation_p2" = "peak_gene.r",
                "region_gene_raw_pvalue_p2" = "peak_gene.p_raw",
  ...
  left_join(pM.conn %>%
              ...
              dplyr::rename("region_gene_correlation_pM" = "peak_gene.r",
                            "region_gene_raw_pvalue_pM" = "peak_gene.p_raw",
            ...
            by = c("gene_region_id" = "gene_region_id")) %>%
  filter(!is.na(region_gene_correlation_pM)) %>%
  dplyr::select(region_id,
                region_coord,
                gene_id,
                gene_name,
                ...
                region_gene_raw_pvalue_p2,
                region_gene_adj_pvalue_p2,
                region_gene_correlation_pM,
                region_gene_raw_pvalue_pM,
                region_gene_adj_pvalue_pM,
                is_dependent_region_pM,
                is_element_pM,
                is_dependent_gene_pM) %>%
  mutate(pair_category = ifelse(is_dependent_region_pM & is_dependent_gene_pM, "Both NFIA-dependent in pM",
                                ifelse(!is_dependent_region_pM & is_dependent_gene_pM, "Region NFIA-independent in pM",
                                       ifelse(is_dependent_region_pM & !is_dependent_gene_pM, "Gene NFIA-independent in pM",
                                              "Both NFIA-independent in pM"))))
...
<!-- Explain the red dots above the diagonal with the box plot of overall correlation coefficients. -->

Find such pM-specific pairs:
...
<!-- Also consider pairs that are significant in both: how many pairs? what are the genes? -->

---

Compare the sets of region-gene pairs in p2 and pMN:

```{r}
# !!Also print as p2-specific element-gene pairs that are technically p2-specific and not present among all pM pairs.
...
Compare the correlation coefficients of the shared and domain-specific region-gene pairs (excluding the only negatively correlated pair):

```{r}
p2.pM.conn.element.dep = p2.conn.element.dep.shared %>%
  mutate(category = "shared_p2") %>%
  bind_rows(pM.conn.element.dep.shared %>%
              mutate(category = "shared_pM")) %>%
  bind_rows(p2.conn.element.dep.specific %>%
              mutate(category = "specific_p2")) %>%
  bind_rows(pM.conn.element.dep.specific %>%
              mutate(category = "specific_pM")) %>%
  filter(peak_gene.r > 0)

p = p2.pM.conn.element.dep %>%
  ggplot(aes(x = category,
             y = peak_gene.r)) +
    geom_boxplot() +
    theme_classic()

ggsave("../r_results/predict_correlated_expressed_gene_granie/plots/p2_pM_pairs_r_distributions.pdf",
       p)

p
```

For either domain, the correlation coefficients of shared and specific region-gene pairs are comparable and hence the shared and specific pairs likely have comparable probabilities of being functional. The fact that the correlation coefficients of both the shared and the specific pairs are lower in p2 than in pMN could be explained by a difference in data quality (as we expect that the degree of correlation in real transcription regulation should not depend on a domain and be an intrinsic property of the fundamental regulation mechanisms).

Compare the correlation coefficients of the p2-specific pairs in p2 and pMN. For functional region-gene pairs that are specific to p2 we expect very low correlation in pMN (not a correlation coefficient that was high but just not high enough to call a region-gene pair significant in pMN):
...
p2.specific.pairids = p2.conn.element.dep.specific %>%
  mutate(pairID = paste0(RegionID, "_", gene.ENSEMBL)) %>%
  pull(pairID)

pM.all.pairids = pM.conn %>%
  mutate(pairID = paste0(RegionID, "_", gene.ENSEMBL)) %>%
  pull(pairID)

p2.specific.pM.all.common.pairids = intersect(p2.specific.pairids,
                                              pM.all.pairids)

p2.pM.df = p2.conn.element.dep.specific %>%
  mutate(pairID = paste0(RegionID, "_", gene.ENSEMBL)) %>%
  filter(pairID %in% p2.specific.pM.all.common.pairids) %>%
  dplyr::select(pairID,
                gene_name,
                RegionID,
                gene.ENSEMBL,
                peak_gene.distance,
                peak_gene.r) %>%
  dplyr::rename("peak_gene.r.p2" = "peak_gene.r") %>%
  left_join(pM.conn %>%
              mutate(pairID = paste0(RegionID, "_", gene.ENSEMBL)) %>%
              filter(pairID %in% p2.specific.pM.all.common.pairids) %>%
              dplyr::select(pairID,
                            peak_gene.r) %>%
              dplyr::rename("peak_gene.r.pM" = "peak_gene.r"),
            by = c("pairID" = "pairID"))


p = p2.pM.df %>%
  ggplot(aes(x = peak_gene.r.p2,
             y = peak_gene.r.pM)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0) +
  lims(x = c(0, 1), y = c(0, 1)) +
...
ggsave("../r_results/predict_correlated_expressed_gene_granie/plots/p2_specific_pairs_in_p2_vs_pM.pdf",
       p)
...
# pM.pairid.in.p2.specific = pM.conn %>%
#   mutate(pairID = paste0(RegionID, "_", gene.ENSEMBL)) %>%
#   filter(pairID %in% p2.conn.element.dep.ids.specific) %>%
#   pull(pairID)
# 
# p2.specific.in.pM.df = p2.pM.conn.element.dep %>%
#   filter(pairID %in% pM.pairid.in.p2.specific) %>%
#   arrange(pairID) %>%
#   dplyr::select(pairID,
#                 peak_gene.r) %>%
#   dplyr::rename("pairID_p2" = "pairID",
#                 "peak_gene.r_p2" = "peak_gene.r") %>%
#   bind_cols(pM.conn %>%
#               mutate(pairID = paste0(RegionID, "_", gene.ENSEMBL)) %>%
#               filter(pairID %in% pM.pairid.in.p2.specific) %>%
#               arrange(pairID) %>%
#               dplyr::select(peak_gene.r) %>%
#               dplyr::rename("peak_gene.r_pM" = "peak_gene.r"))
# 
# p = p2.specific.in.pM.df %>%
#   ggplot(aes(x = peak_gene.r_p2,
#              y = peak_gene.r_pM)) +
#     geom_point() +
#     geom_abline(slope = 1, intercept = 0) +
#     lims(x = c(0, 1), y = c(0, 1)) +
...
# Save p2.specific.in.pM.df
```

-   Correct the interpretation.
-   Print the p2-specific pairs that are not among pM-all.
-   Polish the two plots.
-   Proceed with the plots below (and interpret the higher correlations in p2 than in pM for the pM-specific vs p2-all).

Contrary to our expectations, the majority of the "p2-specific" region-gene pairs are as highly (or even higher!) correlated in pM as in p2. It means that in both domains they have comparably high correlation but in pM they did not pass the FDR cutoff. Consequently, as the only plausible p2-specific connections, we could take the connections with the correlation coefficients in pM less than 0.5:

```{r}
p2.specific.in.pM.df %>%
  arrange(peak_gene.r_pM) %>%
  filter(peak_gene.r_pM < 0.5)
```

Compare the correlation coefficients of the pM-specific pairs in pM and p2. From the result above, we expect that all these pairs will be considerably higher correlated in pM than in p2, simply because significant region-gene pairs are higher correlated in pM than in p2 in general:
...
p2.pairid.in.pM.specific = p2.conn %>%
  mutate(pairID = paste0(RegionID, "_", gene.ENSEMBL)) %>%
  filter(pairID %in% pM.conn.element.dep.ids.specific) %>%
  pull(pairID)

pM.specific.in.p2.df = p2.pM.conn.element.dep %>%
  filter(pairID %in% p2.pairid.in.pM.specific) %>%
  arrange(pairID) %>%
  dplyr::select(pairID,
                peak_gene.r) %>%
  dplyr::rename("pairID_pM" = "pairID",
                "peak_gene.r_pM" = "peak_gene.r") %>%
  bind_cols(p2.conn %>%
              mutate(pairID = paste0(RegionID, "_", gene.ENSEMBL)) %>%
              filter(pairID %in% p2.pairid.in.pM.specific) %>%
              # Exclude the pair that was negatively correlated in pM
              filter(pairID != "Interval_107245_ENSMUSG00000026934") %>% 
              arrange(pairID) %>%
              dplyr::select(peak_gene.r) %>%
              dplyr::rename("peak_gene.r_p2" = "peak_gene.r"))

p = pM.specific.in.p2.df %>%
  ggplot(aes(x = peak_gene.r_pM,
             y = peak_gene.r_p2)) +
    geom_point() +
    geom_abline(slope = 1, intercept = 0) +
    #lims(x = c(0, 1), y = c(0, 1)) +
...
ggsave("../r_results/predict_correlated_expressed_gene_granie/plots/pM_specific_pairs_in_pM_vs_p2.pdf",
       p)

p

# Save pM.specific.in.p2.df
```

Interestingly, there are "pM-specific" pairs that are correlated even higher in p2, although correlations in p2 are, overall, lower than in pM. As reasonable pM-specific, take the pairs with the correlation coefficients in p2 less than 0.5:

```{r}
pM.specific.in.p2.df %>%
  arrange(peak_gene.r_p2) %>%
  filter(peak_gene.r_p2 < 0.5)
```
